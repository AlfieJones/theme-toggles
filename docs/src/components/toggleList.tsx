...
import ToggleMeta from "../../toggles.config";
...
function Toggle({ name }: { name: string }) {
  ...
  return (
    <div className="rounded w-full max-w-[17rem] overflow-hidden shadow hover:shadow-lg transition">
      <ToggleComponent
        className={clsx(
          "relative z-10 block h-full mx-auto dark:text-neutral-900 text-neutral-800 !outline-none p-6 h-52 w-full",
      ...
      />
      <div className="p-4 bg-white rounded-b dark:bg-neutral-800 ">
        <h3 className="block pb-2 text-xl font-semibold text-neutral-700 dark:text-neutral-50">
          {displayName}
        </h3>
        <Link href="#" className="font-semibold text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300">See the code</Link>
      </div>
    </div>
  );
}
